...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> getMessagesList(
      ...
      bool isFromMsgListen = false}) async {
    ...
    try {
      ...
      if (statusCode200Check(response)) {
        //
        if (isFromMsgListen) {
          for (var newItem in (response['data']['messages'] as List)
              .map((message) => msg.MessageGroups.fromJson(message))
              .toList()) {
            int existingIndex =
                messageGroups.indexWhere((item) => item.sId == newItem.sId);
            if (existingIndex != -1) {
          ...
          messageGroups
              .addAll((response['data']['messages'] as List).map((message) {
  ...
  Future<void> getReplyMessageList(
      {required String msgId, required String fromWhere}) async {
  ...
  Future<void> seenReplayMessage({required String msgId}) async {
    final requestBody = {"messageId": msgId};
    final response = await ApiService.instance.request(
        endPoint: ApiString.replayMsgSeen,
        method: Method.POST,
        reqBody: requestBody);
  }
  ...
  Future<List<String>> uploadFiles() async {
    try {
      ...
      List<PlatformFile> selectedFiles =
          FileServiceProvider.instance.selectedFiles;
      List<File> filesToUpload = selectedFiles.map((platformFile) {
        return File(platformFile.path!);
      }).toList();
      print("<<<<<<<<<<SUIIIII>>>>>>>>>>>");
      var request = http.MultipartRequest('POST',
          Uri.parse(ApiString.baseUrl + ApiString.uploadFileForMessageMedia));
  ...
  Future<void> sendMessage({
    ...
    final response = await ApiService.instance.request(
        endPoint: ApiString.sendMessage,
        method: Method.POST,
        reqBody: requestBody);
    print("Send Message requestBody -= $requestBody");
    if (statusCode200Check(response)) {
      /// Socket Emit ///
      socketProvider.sendMessagesSC(
          response: response['data'],
          emitReplyMsg: replyId != null ? true : false);

      /// find where to add ///
      if (editMsgID != null && editMsgID.isNotEmpty) {
        int editIndex = messageGroups.indexWhere(
            (item) => item.messages!.any((msg) => msg.sId == editMsgID));

        if (editIndex != -1) {
          // Update the existing message
          msg.Messages editedMessage = msg.Messages.fromJson(response['data']);
          editedMessage.isEdited = true; // Set isEdited to true
          messageGroups[editIndex].messages![messageGroups[editIndex]
              .messages!
              .indexWhere((msg) => msg.sId == editMsgID)] = editedMessage;
        }
        ...
        int existingIndex =
            messageGroups.indexWhere((item) => item.sId == todayDate);
        if (existingIndex != -1) {
          messageGroups[existingIndex]
              .messages!
              .add(msg.Messages.fromJson(response['data']));
        } else {
          final newListOfDate = response['data'];
  ...
  Future<void> forwardMessage(
      {required Map<String, dynamic> forwardBody}) async {
    final response = await ApiService.instance.request(
        endPoint: forwardBody.keys.contains('channelId')
            ? ApiString.sendChannelMessage
            : ApiString.sendMessage,
        method: Method.POST,
        reqBody: forwardBody,
        needLoader: false);
    if (statusCode200Check(response)) {
      socketProvider.sendMessagesSC(
          response: response['data'], emitReplyMsg: false);
    }
  }
  ...
  Future<void> deleteMessage(
    ...
    final response = await ApiService.instance.request(
        endPoint: ApiString.deleteMessage + messageId, method: Method.DELETE);
    if (statusCode200Check(response)) {
      // getMessagesList(oppositeUserId: receiverId);
  ...
  Future<void> deleteMessageForReply(
      {required String messageId, required firsMessageId}) async {
    final response = await ApiService.instance.request(
        endPoint: ApiString.deleteMessage + messageId, method: Method.DELETE);
    if (statusCode200Check(response)) {
  ...
  Future<void> pinUnPinMessage(
      {required String receiverId,
      required String messageId,
      required bool pinned}) async {
    final response = await ApiService.instance.request(
        endPoint: ApiString.pinMessage(messageId, pinned), method: Method.PUT);
    if (statusCode200Check(response)) {
      // getMessagesList(oppositeUserId: receiverId);
      togglePinModel(messageId);
      // _updatePinnedStatus(messageId, pinned);
      socketProvider.pinUnPinMessageEvent(
          senderId: signInModel.data?.user?.id ?? "",
          receiverId: receiverId,
          isEmitForChannel: false);
    }
  }

  Future<void> pinUnPinMessageForReply(
      {required String receiverId,
      required String messageId,
      required bool pinned}) async {
    final response = await ApiService.instance.request(
  ...
  void deleteMessageFromReplyModel(String messageId) {
    for (var messageGroup in getReplyMessageModel?.data?.messages ?? []) {
      messageGroup.groupMessages
          ?.removeWhere((message) => message.sId == messageId);
    }
    notifyListeners();
  }
  ...
  void togglePinModel(String messageId) {
    for (var messageGroup in messageGroups ?? []) {
      for (var message in messageGroup.messages ?? []) {
        if (message.sId == messageId) {
          message.isPinned =
              !(message.isPinned ?? false); // Toggle the isPinned status
  ...
  Future<void> getFileListingInChat({required String oppositeUserId}) async {
    ...
    final response = await ApiService.instance.request(
        endPoint: ApiString.getFileListingInChat,
        method: Method.POST,
        reqBody: requestBody);
    if (statusCode200Check(response)) {
